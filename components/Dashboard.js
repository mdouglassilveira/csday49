...
function MetCard({ label, value, sub, color, accent }) {
  return (
    <div style={{ ...card, borderLeft: accent ? `3px solid ${color||'var(--orange)'}` : '1px solid var(--border)', borderTop: 'none', borderRight: 'none', borderBottom: 'none', border: `1px solid var(--border)`, borderLeft: accent ? `3px solid ${color||'var(--orange)'}` : `1px solid var(--border)` }}>
      <div style={sTitle}>{label}</div>
      <div style={{ fontFamily:'var(--font-title)', fontSize:38, lineHeight:1, color: color||'var(--txt)', letterSpacing:'.02em', marginBottom:4 }}>{value}</div>
      {sub && <div style={{ fontSize:10, color:'var(--txt-3)', fontFamily:'var(--font-body)', fontWeight:400 }}>{sub}</div>}
    </div>
  )
}
...
function AlertStrip({ startups, onSelect }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:16, flexShrink:0 }}>
      {criticos.length > 0 && (
        <div style={{ background:'var(--red-dim)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:8, padding:'9px 14px', display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
          <span style={{ fontSize:10, fontWeight:700, color:'var(--red)', whiteSpace:'nowrap', flexShrink:0, fontFamily:'var(--font-body)', letterSpacing:'.06em' }}>SEM PRESENÇA · {criticos.length}</span>
          <div style={{ width:1, height:14, background:'rgba(239,68,68,0.3)', flexShrink:0 }}/>
          {criticos.slice(0,8).map(s=>(
            <button key={s.startup_id} onClick={()=>onSelect(s)} style={{ fontSize:10, padding:'3px 10px', background:'transparent', color:'var(--red)', border:'1px solid rgba(239,68,68,0.4)', borderRadius:4, cursor:'pointer', fontFamily:'var(--font-body)', fontWeight:500, whiteSpace:'nowrap', transition:'all .15s' }}
      ...
      {riscos.length > 0 && (
        <div style={{ background:'var(--amber-dim)', border:'1px solid rgba(245,158,11,0.3)', borderRadius:8, padding:'9px 14px', display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
          <span style={{ fontSize:10, fontWeight:700, color:'var(--amber)', whiteSpace:'nowrap', flexShrink:0, fontFamily:'var(--font-body)', letterSpacing:'.06em' }}>EM RISCO · {riscos.length}</span>
          <div style={{ width:1, height:14, background:'rgba(245,158,11,0.3)', flexShrink:0 }}/>
          {riscos.slice(0,8).map(s=>(
            <button key={s.startup_id} onClick={()=>onSelect(s)} style={{ fontSize:10, padding:'3px 10px', background:'transparent', color:'var(--amber)', border:'1px solid rgba(245,158,11,0.4)', borderRadius:4, cursor:'pointer', fontFamily:'var(--font-body)', fontWeight:500, whiteSpace:'nowrap', transition:'all .15s' }}
  ...
  return (
    <div style={card}>
          ...
          return (
            <div key={sp.n} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:5 }}>
              ...
              <div style={{ width:'100%', display:'flex', gap:2, alignItems:'flex-end', height:110 }}>
                <div title={`Workshop: ${sp.workshop}`}   style={{ flex:1, height:`${hW}%`, background:'var(--orange)', borderRadius:'3px 3px 0 0', minHeight:2, boxShadow:`0 0 8px var(--orange-glow)` }} />
                <div title={`Mentoria: ${sp.mentoria}`}   style={{ flex:1, height:`${hM}%`, background:'#FF8C4B', borderRadius:'3px 3px 0 0', minHeight:2, opacity:.75 }} />
                <div title={`Atividade: ${sp.atividade}`} style={{ flex:1, height:`${hA}%`, background:'var(--bg-5)', borderRadius:'3px 3px 0 0', minHeight:2, border:'1px solid var(--border-3)' }} />
              </div>
      ...
      <div style={{ display:'flex', gap:20, marginTop:16, paddingTop:12, borderTop:'1px solid var(--border)' }}>
        {[
          { color:'var(--orange)', label:'Workshop (presença)' },
          { color:'#FF8C4B',       label:'Mentoria (presença)' },
          ...
          <div key={l.label} style={{ display:'flex', alignItems:'center', gap:7, fontSize:11, color:'var(--txt-3)', fontFamily:'var(--font-body)', fontWeight:400 }}>
            <div style={{ width:10, height:10, background:l.color, borderRadius:2, flexShrink:0, border:l.border||'none' }} />
            {l.label}
          </div>
        ))}
      </div>
