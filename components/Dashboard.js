...
import { programMetrics, pct, riskColor, riskLabel, sprintRate, autoRiskLevel } from '../lib/metrics'
  ...
  return (
    <div style={{ ...card, gridColumn: '1 / -1' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 120 }}>
        {data.map((sp) => {
          const hSprint   = maxVal > 0 ? (sp.sprint   / maxVal) * 100 : 0
          ...
          return (
            <div key={sp.n} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
              <div style={{ fontSize: 9, color: 'var(--gray-4)', fontFamily: 'var(--font-m)', marginBottom: 2 }}>{sp.sprint}</div>
              <div style={{ width: '100%', display: 'flex', gap: 2, alignItems: 'flex-end', height: 90 }}>
                <div title={`Sprint: ${sp.sprint}`}   style={{ flex: 1, height: `${hSprint}%`,   background: 'var(--orange)', borderRadius: '2px 2px 0 0', minHeight: 2 }} />
                <div title={`Workshop: ${sp.workshop}`} style={{ flex: 1, height: `${hWorkshop}%`, background: 'var(--blue)',   borderRadius: '2px 2px 0 0', minHeight: 2, opacity: .8 }} />
                <div title={`Mentoria: ${sp.mentoria}`} style={{ flex: 1, height: `${hMentoria}%`, background: '#7C3AED',       borderRadius: '2px 2px 0 0', minHeight: 2, opacity: .7 }} />
              </div>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 10 }}>
        {[
          { color: 'var(--orange)', label: 'Sprint' },
          { color: 'var(--blue)',   label: 'Workshop' },
          { color: '#7C3AED',       label: 'Mentoria' },
        ].map((l) => (
          ...
          <div key={gt}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: pct(data.avgPresence) >= 60 ? 'var(--green)' : pct(data.avgPresence) >= 40 ? 'var(--amber)' : 'var(--red)', fontFamily: 'var(--font-m)' }}>{pct(data.avgPresence)}%</span>
              </div>
            </div>
            <Bar value={pct(data.avgPresence)} max={100} color={pct(data.avgPresence) >= 60 ? 'var(--green)' : pct(data.avgPresence) >= 40 ? 'var(--amber)' : 'var(--red)'} height={6} />
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {bySegmento.map((s) => (
          <div key={s.seg} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <Bar value={pct(s.avgPresence)} max={100} color={pct(s.avgPresence) >= 60 ? 'var(--green)' : pct(s.avgPresence) >= 40 ? 'var(--amber)' : 'var(--red)'} height={7} />
            <div style={{ width: 32, textAlign: 'right', fontSize: 11, color: 'var(--gray-3)', fontFamily: 'var(--font-m)', flexShrink: 0 }}>{pct(s.avgPresence)}%</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {byRegiao.map((r) => (
          <div key={r.reg} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <Bar value={pct(r.avgPresence)} max={100} color={pct(r.avgPresence) >= 60 ? 'var(--green)' : pct(r.avgPresence) >= 40 ? 'var(--amber)' : 'var(--red)'} height={7} />
            <div style={{ width: 32, textAlign: 'right', fontSize: 11, color: 'var(--gray-3)', fontFamily: 'var(--font-m)', flexShrink: 0 }}>{pct(r.avgPresence)}%</div>
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 14 }}>
        ...
        <MetCard label="Presença média"       value={`${pct(m.avgSprint)}%`}    sub="nos sprints realizados"        color={pct(m.avgSprint) >= 60 ? 'var(--green)' : 'var(--amber)'} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 14 }}>
        ...
        <MetCard label="Em risco ou crítico" value={(m.riskMap.risco || 0) + (m.riskMap.critico || 0)} sub="precisam de atenção urgente" color="var(--red)" />
      </div>
