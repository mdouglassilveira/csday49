...
// Parse date strings (dd/mm) to Date objects (year 2026)
function parseDate(str) {
  const [d, m] = str.split('/')
  return new Date(2026, parseInt(m) - 1, parseInt(d))
}

// Get event info for today or the most recent/next event
export function getCurrentEvent() {
  ...
  for (const sp of SPRINTS) {
    if (sp.status === 'fut') continue
    const wkDate = parseDate(sp.wk)
    const mtDate = parseDate(sp.mt)
    if (wkDate.getTime() === today.getTime()) return { sprint: sp, type: 'workshop', date: wkDate }
    if (mtDate.getTime() === today.getTime()) return { sprint: sp, type: 'mentoria', date: mtDate }
  }
  ...
  for (const sp of SPRINTS) {
    ...
    const wkDate = parseDate(sp.wk)
    const mtDate = parseDate(sp.mt)
    if (wkDate <= today) lastEvent = { sprint: sp, type: 'workshop', date: wkDate }
    if (mtDate <= today) lastEvent = { sprint: sp, type: 'mentoria', date: mtDate }
  }
  return lastEvent || { sprint: SPRINTS[0], type: 'workshop', date: parseDate(SPRINTS[0].wk) }
}
...
export function getNextEvent() {
  ...
  for (const sp of SPRINTS) {
    const wkDate = parseDate(sp.wk)
    const mtDate = parseDate(sp.mt)
    if (wkDate > today) return { sprint: sp, type: 'workshop', date: wkDate }
    if (mtDate > today) return { sprint: sp, type: 'mentoria', date: mtDate }
  }
  return null
}
